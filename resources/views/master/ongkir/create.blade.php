...
@section('content')
    ...
    <section class="content">
        <div class="box box-success">
            ...
            <div class="box-body">
                <form class="form-horizontal" action="{{ route('ongkir.store') }}" method="POST">
                    @csrf
                            ...
                            <select class="form-control select2 @error('city_id') is-invalid @enderror" id="city_id"
                                ...
                                @foreach ($cities as $city)
                                    <option value="{{ $city->id }}"
                                        @if (old('city_id') == $city->id) selected="selected" @endif> {{ $city->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('city_id')
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('biaya') is-invalid @enderror" id="biaya"
                                name="biaya" value="{{ old('biaya') }}" placeholder="Biaya Pengiriman">
                            @error('biaya')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
